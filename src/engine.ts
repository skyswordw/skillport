...
const WEIGHT: Record<Severity, number> = { error: 40, warning: 15, info: 5 };
const STATUS_ORDER: Record<AgentStatus, number> = { works: 0, warns: 1, breaks: 2 };
...
export function lintSkill(skill: ParsedSkill, targets: AgentId[], rules: Rule[] = RULES): SkillReport {
  ...
  const worst = agents.reduce<AgentVerdict | null>(
    (acc, a) => (acc === null || a.score < acc.score ? a : acc),
    null
  );
  const overallStatus = agents.reduce<AgentStatus>(
    (acc, a) => (STATUS_ORDER[a.status] > STATUS_ORDER[acc] ? a.status : acc),
    "works"
  );
